...
export class Camera {
  ...
  constructor(
    ...
    const viewportHeight =
      2.0 * options.focusDist * Math.tan(degreeToRadian(options.vFov) / 2);
    ...
    // 计算散焦参数
    const defocusRadius =
      options.focusDist * Math.tan(degreeToRadian(options.defocusAngle / 2));
  ...
  private generateOrigin() {
    ...
    return this.cameraOrigin
      .add(this.defocusDiskU.mul(p.x))
      .add(this.defocusDiskV.mul(p.y));
  }

  render(ctx: CanvasRenderingContext2D, world: Hittable): AbortController {
  ...
  private rayTrace(ray: Ray, world: Hittable, depth: number): Color {
    ...
    if (hitRecord) {
      ...
      const emittedResult =
        hitRecord.material.emitted?.(hitRecord.u, hitRecord.v, hitRecord.p) ||
        Vec3.ZERO;
